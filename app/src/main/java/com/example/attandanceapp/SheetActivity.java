...
public class SheetActivity extends AppCompatActivity {
    ...
    private void showTable() {
        ...
        for (int i = 1; i < rowSize; i++) {  // Fix the loop condition
            roll_tvs[i].setText(String.valueOf(rollArray[i - 1]));
            name_tvs[i].setText(nameArray[i - 1]);

            for (int j = 1; j <= DAY_IN_MONTH; j++) {  // This loop starts from 1, so it's okay
        ...
        for (int i = 0; i < idArray.length + 1; i++) {  // This ensures correct indexing
    ...
    private int getDayInMonth(String month) {
        ...
        int monthIndex = Integer.valueOf(month.substring(0, 2)) - 1;  // Fix: Use 2 digits for month
